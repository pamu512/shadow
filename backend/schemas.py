...
class SimulateRepresentmentRequest(BaseModel):
    ...
    transaction_id: str | None = Field(default=None, description="Row-level manifest; omit for cohort-only review.")


class AtoUserRequest(BaseModel):
    user_id: str = Field(..., min_length=1)
    user_column: str | None = Field(default=None, description="Override auto-detected user id column name.")
...
class AtoAnalyzeRequest(BaseModel):
    ...
    user_id: str | None = Field(default=None, description="Account id; omit for schema-based default (most common id).")
    user_column: str | None = Field(default=None, description="Override auto-detected user id column name.")
...
class AgentQueryRequest(BaseModel):
    case_id: str
    sql: str = Field(..., min_length=1, description="SELECT or WITH query against the case DuckDB dataset table.")
