...
**Cross-cutting topics** (security, scale, ingestion): warehouse **ACL views** and **case shares**, **async** ORM usage in HTTP routers, **Docker storage-opt** probing, per-run **sandbox cwd** isolation, **CSV delimiter sniffing**, and optional **ONNX** tool-confidence training are documented in [CONFIGURATION.md](./CONFIGURATION.md), [ARCHITECTURE.md](./ARCHITECTURE.md), and the root [README.md](../README.md).